...
export function PayZoneAdjustment({
  ...
  const renderPayZoneLevelGroup = (payZone: number, level: string) => {
    ...
    return (
      <div key={groupKey} className="bg-white rounded-lg shadow-lg overflow-hidden mb-3">
        {/* 헤더 */}
        <div 
          className={`px-3 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 border-b cursor-pointer hover:opacity-90 transition-opacity`}
          onClick={() => toggleGroup(groupKey)}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded text-xs font-bold">
                  Pay Zone {payZone}
                </span>
                <span className="px-2 py-0.5 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded text-xs font-bold">
                  {level}
                </span>
              </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="px-2 py-2 text-center text-xs font-semibold text-gray-700 w-20">총계</th>
                </tr>
              </thead>
              <tbody>
                {/* Base-up 행 */}
                ...
                <tr className="border-b hover:bg-gray-50 transition-colors">
                  <td className="px-3 py-2 text-xs font-medium text-gray-700">
                    Merit (%)
                    <div className="text-xs text-gray-500">기본값</div>
                  </td>
                  {performanceGrades.map(grade => {
                    const rates = pendingPayZoneRates[payZone]?.[level]?.[grade] || { baseUp: 0, merit: 0, additional: 0 }
                    const weight = performanceWeights[grade] || 1.0
                    ...
                    return (
                      <td key={grade} className={`px-2 py-2 text-center ${
                        ...
                        {additionalType === 'amount' && rates.additional > 0 && (
                          <div className="text-xs text-gray-600">+{rates.additional}만</div>
                        )}
                      </td>
                    )
                  })}
    ...
    <div className="space-y-2">
      ...
      <div className="bg-white rounded-lg shadow-sm px-3 py-2 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-500">Pay Zone-레벨-평가등급별 세밀한 조정</span>
        </div>
      ...
      {/* 일괄 조정 도구 */}
      <div className="bg-white rounded-lg shadow-sm px-3 py-2 flex items-center justify-between">
        <span className="text-xs font-medium text-gray-700">전체 일괄 설정</span>
        <div className="flex gap-3">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              step="0.1"
              className="w-16 px-2 py-1 text-xs text-center border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
            />
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              step="0.1"
              className="w-16 px-2 py-1 text-xs text-center border border-gray-300 rounded focus:ring-1 focus:ring-green-500 focus:border-green-500"
            />
            <span className="text-xs text-gray-500">%</span>
          </div>
          <button
